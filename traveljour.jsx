...
import { 
  ...
  X, LogIn, ChevronDown, PenSquare
} from 'lucide-react';

// --- MOCK INITIAL DATA (Matches your UI Design) ---
...
export default function App() {
  ...
  const [loginType, setLoginType] = useState(null); // 'resume' or 'blog'
  ...
  const handleLogin = (e) => {
    ...
    if (passwordInput === 'resume' || passwordInput === 'blog') {
      ...
      setAuthError("");
      // In a real DB, we would save this login event to track access
      console.log(`Access logged: User entered via [${passwordInput}] password.`);
    } else {
  ...
  const handleSaveEntry = (e) => {
    ...
    // Simulate Unsplash API Fallback if no photo URL provided
    let photoUrl = formData.get('photoUrl');
    if (!photoUrl) {
      // Using a reliable generic travel Unsplash image if empty
      photoUrl = `https://images.unsplash.com/photo-1488085061387-422e29b40080?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80`;
    }
  ...
  const heroEntry = filteredEntries[0]; // Featured trip is the most recent/relevant
  const recentEntries = filteredEntries.slice(1, 5); // The rest of the trips
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-800">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-slate-100 p-6">
        ...
        <nav className="flex-1 space-y-2">
          ...
          <NavItem icon={<Briefcase size={20} />} label="Trips" />
          <NavItem icon={<MapIcon size={20} />} label="Map" />
          <NavItem icon={<BookOpen size={20} />} label="Journal" />
          <NavItem icon={<ImageIcon size={20} />} label="Photos" />
          <NavItem icon={<Heart size={20} />} label="Bucket List" />
        ...
        <div className="mt-auto">
          <div className="flex items-center gap-3 mb-6 p-2 hover:bg-slate-50 rounded-xl cursor-pointer transition-colors">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500">Logged in via: {loginType}</p>
            </div>
      ...
      <main className="flex-1 px-4 md:px-8 py-8 overflow-y-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-serif text-slate-800">Good morning, Alex!</h1>
            <p className="text-slate-500 mt-1">Where will your next adventure be?</p>
          </div>
        ...
        <div className="flex justify-between items-end mb-6">
          ...
          <button className="text-blue-600 text-sm font-medium hover:underline">View all trips &rsaquo;</button>
        </div>
      ...
      <aside className="hidden xl:block w-80 bg-white border-l border-slate-100 p-6 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3 mb-10">
          ...
          <StatCard icon={<MapPin size={20} className="text-amber-500" />} bg="bg-amber-50" value="23" label="Places Visited" />
        ...
        <div className="flex justify-between items-end mb-4">
          ...
          <button className="text-slate-400 text-xs font-medium hover:text-blue-600">View map</button>
        </div>
        <div className="bg-slate-50 rounded-2xl p-4 mb-10 flex items-center justify-center min-h-[160px] border border-slate-100">
          {/* Simulated Map Graphic */}
          <div className="relative w-full h-full opacity-60">
             <svg viewBox="0 0 100 50" className="w-full h-full text-slate-300 fill-current">
                <path d="M10,20 Q15,10 25,15 T40,25 T60,15 T80,20 T95,30 Q85,40 70,35 T45,30 T25,40 T5,30 Z" />
             </svg>
             <div className="absolute top-1/4 left-1/4 w-2 h-2 bg-blue-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(59,130,246,0.8)]"></div>
             <div className="absolute top-1/2 left-1/2 w-2 h-2 bg-blue-500 rounded-full shadow-[0_0_10px_rgba(59,130,246,0.8)]"></div>
             <div className="absolute top-1/3 right-1/4 w-2 h-2 bg-blue-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(59,130,246,0.8)]"></div>
          </div>
        </div>

        <div className="flex justify-between items-end mb-4">
          <h3 className="font-semibold text-slate-800">Recent Entries</h3>
          <button className="text-slate-400 text-xs font-medium hover:text-blue-600">View all</button>
        </div>
        <div className="space-y-4">
          {entries.slice(0,4).map(entry => (
            <div key={entry.id} className="flex gap-3 group cursor-pointer" onClick={() => openModal(entry)}>
              ...
              <div>
                <h5 className="text-sm font-semibold text-slate-800 leading-tight group-hover:text-blue-600 transition-colors">{entry.title}</h5>
...
function NavItem({ icon, label, active }) {
  return (
    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all ${active ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}>
      {icon}
